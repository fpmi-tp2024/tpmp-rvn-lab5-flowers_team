...
{
    SQLite::Statement query(db, "SELECT f.variety AS variety_name, SUM(cf.flower_count * o.composition_count) AS flower_count \
    FROM \"Order\" o JOIN Composition_flower cf ON o.composition_id = cf.composition_id JOIN Flower f ON cf.id_flower = f.id \
    WHERE o.release_date BETWEEN ? AND ? GROUP BY f.variety;");
...
FlowerVarietyPeriodAnalyzer::~FlowerVarietyPeriodAnalyzer() = default;